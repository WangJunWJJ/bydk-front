...
export class ModelRLConfigViewComponent implements OnInit {
  record!: {
    id: string;
  };
  ...
  ngOnInit(): void {
    zip(this.modelConfigService.getRLMission(this.record.id), this.modelConfigService.getRLMonitorUrl(this.record.id)).subscribe(
      ([mission, url]) => {
