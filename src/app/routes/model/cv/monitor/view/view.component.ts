...
import { ClusterLogTypeEnum, IMission, ICVConfig, ISlaveData, MissionData, PyClusterResponse } from 'src/app/core/service/project/core';
...
export class ModelCVPyClusterMonitorComponent implements OnInit, OnDestroy, AfterViewInit {
  record!: {
    id: string;
  ...
  ngOnInit(): void {
    this.missionName = this.record?.mission?.name ?? '';
    console.log(this.record.id); // TODO loaddata
              ...
              return timer(0, 1000).pipe(
                ...
                switchMap(_ => {
                  ...
                  if (config === null) {
                    ...
                    return this.pyClusterService.getSlaveData(monitorUrl, config);
                  }
                })
              );
            })
